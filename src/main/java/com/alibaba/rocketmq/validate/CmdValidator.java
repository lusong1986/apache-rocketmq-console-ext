			...
			for (Method method : methodArr) {
				if (method.isAnnotationPresent(CmdTrace.class)) {
					...
					if (method2cmd.get(methodName) == null) {
						...
						throw new IllegalStateException(methodName + " = {" + method2cmd.get(methodName).getName()
								+ "," + cmdClazz.getName() + "}");
					}
					if (cmd2method.get(cmdClazz) == null) {
						cmd2method.put(cmdClazz, methodName);
					} else {
						throw new IllegalStateException(cmdClazz + " = {" + cmd2method.get(cmdClazz) + "," + methodName
								+ "}");
					}
				}
			}
		...
		if (hasChecked.compareAndSet(false, true)) {
			for (SubCommand cmd : MQAdminStartup.subCommandList) {
				if (cmd2method.containsKey(cmd.getClass())) {
					logger.info("cmdClazz:{}, method:{}", cmd.getClass().getName(), cmd2method.get(cmd.getClass()));
				} else {
					if (isThrowDone()) {
						throw new IllegalStateException("cmdClazz:" + cmd.getClass().getName() + ", method not found");
					} else {
